...
3. **Los casos de uso se podran exponer como skills de IA sin reescribirlos**
   (Fase 12). Ya no dependen de HTTP.
...
Esa uniformidad es tambien la preparacion para la Fase 12: cada caso de uso ya
...
se expongan por MCP (Fase 13), ese canal hara su propia traduccion sin tocar el
negocio.
...
Agregar una tabla nueva (Fase 10) es un procedimiento mecanico que no toca nada
de lo existente:
